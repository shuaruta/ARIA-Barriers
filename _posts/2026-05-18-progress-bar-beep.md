...
W3C の ARIA Authoring Practices Guide (APG) は、`aria-valuemin` と `aria-valuemax` からパーセンテージを計算して支援技術に提示することを求めています。しかし、ビープ音のピッチ計算のような具体的な出力方法までは規定しておらず、実装はスクリーンリーダー側の裁量に委ねられています。
