...
デモを作ったので、ぜひ体験してください。ただし、架空の例を題材にしており、特定の金融機関やサービスの UI を再現しているわけではありません。
...
同じ動向として、2026年には Google が Chrome Canary で「Gmail OTP Autofill」機能を試験しています。Gmail にログインした同一プロファイルで、サイトの OTP 入力欄にメールの確認コードを提案するものです。ただし、このフラグはデフォルトでオフであり、安定版ではなく、実機テストでは未動作の報告もあるため、あくまで動向の注記です。

なお、これらはサイト側の適合とは別の話です。たとえブラウザや OS が自動入力を提案しても、入力欄が貼り付けを拒否したり、パスワードマネージャーや自動入力を妨げたりしていれば、ユーザーは支援を受けられません。3.3.8 で想定される「補助メカニズム」は、コンテンツ側がコピー＆ペーストや自動入力を妨げないことが前提です。また、スクリーンリーダー利用者にとって自動入力がどう通知されるか、手動確認が不要かどうかは未確定です。
...
なお、いい例でも記憶・転記の負荷は残るため、認証に画像を使うこと自体は推奨されません。金融サービスには高いセキュリティと正しいアクセシビリティ理解の両方が求められ、次期 JIS 改正や合理的配慮の義務を踏まえ、認知に依存する認証を避ける設計が求められます。
